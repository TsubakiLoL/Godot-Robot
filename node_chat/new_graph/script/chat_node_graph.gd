...
func init():
	if real_var!=null:
		...
		for i in range(real_var.variable_name_array.size()):
			var new_hbox=preload("res://node_chat/new_graph/tscn/chat_node_graph_hbox.tscn").instantiate()
			add_child(new_hbox)
			hbox_array.append(new_hbox)
			match real_var.variable_type_array[i]:
				ChatNode.variable_type.TYPE_STRING:
					var text_edit=preload("res://node_chat/new_graph/tscn/text_edit.tscn").instantiate() as NodeChatTextInput
				...
				ChatNode.variable_type.TYPE_BOOL:
					var check=preload("res://node_chat/new_graph/tscn/check_button.tscn").instantiate() as CheckButton
				...
				ChatNode.variable_type.TYPE_SELECT:
					...
					var option=preload("res://node_chat/new_graph/tscn/option_button.tscn").instantiate() as NodeChatSelect
				...
				ChatNode.variable_type.TYPE_COLOR:
					var color_picker = preload("res://node_chat/new_graph/tscn/color_picker.tscn").instantiate()
					var value=real_var.get(real_var.variable_name_array[i])
